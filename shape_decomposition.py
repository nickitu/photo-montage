...
# 这个函数的功能是将输入的图像转换为轮廓多边形。它支持有洞的多边形和多个多边形。
# 黑色的部分被认为是多边形的区域。
def generate_canvas_polygon(img, complement=False):
    ...
    # 将图像转换为灰度图，如果图像有多个通道的话。
    if len(img.shape) >= 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 获取图像的高度和宽度，以及总面积。
    img_h, img_w = img.shape
    total_area = img_h * img_w
    # 将图像沿y轴翻转，这是为了和opencv的坐标系保持一致。
    ...
    # 对图像进行二值化，将像素值大于阈值的设为255，小于等于阈值的设为0。
    _, img = cv2.threshold(img, threshold, 255, cv2.THRESH_BINARY)
    
    # 根据complement参数，决定是否对图像进行反色，即将白色和黑色像素互换。这是为了让多边形的区域为白色，背景为黑色。
    if not complement:
    ...
    # 轮廓的格式是(x, y)，x在宽度维度，y在高度维度，顶部为零。
    contour, hierarchy = cv2.findContours(img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    # 创建一个空的多边形列表，用于存储找到的多边形。
    polygons = []
    # 将层次结构转换为一维数组，方便遍历。
    hierarchy = np.reshape(hierarchy, (-1, 4))
    # 遍历轮廓和层次结构，创建多边形对象。这里使用的是Polygon类，它可以表示有洞的多边形。它的参数是一个外部轮廓和一个内部轮廓的列表。
    for c in enumerate(hierarchy):
        # 创建一个空的列表，用于存储内部轮廓。
        holes = []
        # 判断是否是有效的多边形，即是否有子轮廓或者没有父轮廓。
        if(c[1][2] >= 0 or (c[1][2] == -1 and c[1][3] == -1)):
            ...
            # 检查外部轮廓是否有至少三个点，如果有，那么创建一个多边形对象，将外部轮廓和内部轮廓作为参数传入。
            if np.squeeze(contour[c[0]]).shape[0] > 3:
                polygon = Polygon(np.squeeze(contour[c[0]]), holes)
                # 检查多边形的面积是否大于总面积乘以面积过滤比例，如果是，那么将多边形添加到多边形列表中。
