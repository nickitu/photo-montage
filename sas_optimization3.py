...
def process_image_for_optimization(image_dict):
    ...
    for idct in image_dict:
        ...
        rgb0=(idct['rgb'][0]+idct['rgb'][1]+idct['rgb'][2])/3/255
        images.append({'id': index, 'aspect_ratio': (aspect_ratio+rgb0)/2})
...
'''
Initializes a slice tree for each part, hence a forest.
'''
...
def optimization(input_shape, input_mask_folder, input_image_collection_folder,output_dir):
    image = cv2.imread(input_shape, cv2.IMREAD_GRAYSCALE)
    polygon = sd.generate_canvas_polygon(image)[0]
    with open(join(output_dir, 'final_cut_opt.json')) as f:
    ...
    plt.savefig(join(output_dir, '2-1 raw_parts.png'), bbox_inches='tight')
    ...
    for image_id in image_ids:
        ...
        imageColo = load_mask(join(input_image_collection_folder, image_id + ".jpg"))
        
        
        # # labelGray=cv2.cvtColor(label,cv2.COLOR_BGR2GRAY)
        th,thLabel=cv2.threshold(label,200,255,0)
        rgbMean=cv2.mean(imageColo,thLabel)
    ...
    plt.savefig(join(output_dir, '2-2 optimal_layout.png'), bbox_inches='tight')
    ...
    with open(join(output_dir, 'slicing_result.json'), 'w') as f:
        json.dump(optimization_output, f,indent=4)
        print(len(optimization_output))
...
if __name__ == '__main__':
    ...
    scaling_factor=2
    optimization(input_shape, input_mask_folder, input_image_collection_folder, output_dir)
