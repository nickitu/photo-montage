...
def render_collage(input_image_collection_folder, output_dir, scaling_factor):
    ...
    for id,img_dict in enumerate(layout['images']):
        ...
        # shape0=whole_canvas.shape
        patch_origin=list(patch_origin)
        temp=patch_origin[0]+patch.shape[0]
        if temp>whole_canvas.shape[0]:
            patch_origin[0]=patch_origin[0]+(whole_canvas.shape[0]-temp)
            
        temp=patch_origin[1]+patch.shape[1]
        if temp>whole_canvas.shape[1]:
            patch_origin[1]=patch_origin[1]+(whole_canvas.shape[1]-temp)
            
  
        whole_canvas[patch_origin[0]:patch_origin[0]+patch.shape[0],patch_origin[1]:patch_origin[1]+patch.shape[1]] += patch
        # print(id)
        # myshow(whole_canvas,"whole_canvas")
        # cv2.waitKey(0)
    #main_fig, main_ax = plt.subplots(nrows=1, ncols=1, num='Layout', figsize=(18, 18))
    write_color_image(whole_canvas, join(output_dir, '3-1 collage.png'))
    ...
    write_color_image(canvas_border, join(output_dir, '3-2 collage_white_space.png'))
...
if __name__ == '__main__':
    ...
    input_shape="input_data/layout/fly.png"
