...
# The function of this function is to convert the input image into a contours polygon. It supports polygons with holes and multiple polygons.
# The black part is considered the area of the polygon.
def generate_canvas_polygon(img, complement=False):
# Define some parameters, such as threshold, area filter ratio, etc.
    threshold = 127
    area_filter = 0.01
    
    # Convert the image to grayscale if the image has multiple channels.
    if len(img.shape) >= 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # Get the height and width of the image, as well as the total area.
    img_h, img_w = img.shape
    total_area = img_h * img_w
    # Flip the image along the Y-axis to align with opencv's coordinate system.
    ...
    # The image is binarized, and the pixel value greater than the threshold is set to 255, and the pixel value less than or equal to the threshold is set to 0.
    _, img = cv2.threshold(img, threshold, 255, cv2.THRESH_BINARY)
    
    # complement parameter determines whether to invert the image by interchanging white and black pixels. This is to make the area of the polygon white and the background black.
    if not complement:
        img = cv2.bitwise_not(img) # Invert white and black pixels
    
    # Gaussian blur is applied to the image to remove noise.
    img = cv2.GaussianBlur(img, (3,3), sigmaX=0.2, borderType=cv2.BORDER_DEFAULT)
    
    # 使用opencv的findContours函数，找出图像中的轮廓和层次结构。这里使用的模式是RETR_CCOMP，表示只检测外部轮廓和内部轮廓，忽略内部轮廓的内部轮廓。这里使用的方法是CHAIN_APPROX_SIMPLE，表示只保留轮廓的拐点，省略中间的点。
    # The format of the outline is (x, y), with x in the width dimension, y in the height dimension, and zero at the top.
    contour, hierarchy = cv2.findContours(img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    # Creates an empty polygon list to store the found polygons.
    polygons = []
    # Converts the hierarchy into a one-dimensional array for easy traversal.
    hierarchy = np.reshape(hierarchy, (-1, 4))
    # Iterate through contours and hierarchies to create polygon objects. The Polygon class is used here, which can represent polygons with holes. Its arguments are a list of an external outline and an internal outline.
    for c in enumerate(hierarchy):
        # Creates an empty list to store the internal outline.
        holes = []
        # Determines whether a polygon is valid, that is, whether it has child or no parent contour.
        if(c[1][2] >= 0 or (c[1][2] == -1 and c[1][3] == -1)):
            # Gets the index of the first subcontour.
            next_child = c[1][2] 
            # Extract all the suboutlines and place them in the list.
            while next_child >= 0:
                holes.append(np.squeeze(contour[next_child]))
                # Gets the index of the next subcontour.
                next_child = hierarchy[next_child][0]
            # Check if the outer outline has at least three points, and if it does, then create a polygon object, passing in the outer and inner Outlines as parameters.
            if np.squeeze(contour[c[0]]).shape[0] > 3:
                polygon = Polygon(np.squeeze(contour[c[0]]), holes)
                # Check if the area of the polygon is greater than the total area multiplied by the area filter ratio, if so, then add the polygon to the list of polygons.
...
def generate_cuts(filename, output_path):
    ...
    if ma_ext[0].sum() > 0:
        ...
        plt.savefig(join(output_path, '1-1 raw.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-2 representitive.png'), bbox_inches='tight')
        plt.clf()

        # plot corners
        plot_corners(polygon, component_adjusted_ext, image.shape[1], image.shape[0], plot_component = False)
        plt.savefig(join(output_path, '1-3 corner.png') , bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-4 protrusion.png'), bbox_inches='tight')
        plt.clf()

        # Extension strength
        plot_corners(polygon, ec_adjusted, image.shape[1], image.shape[0], plot_component = False)
        plt.savefig(join(output_path,'1-5 extended_corner.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-6 extension.png'), bbox_inches='tight')
        plt.clf()
        ...
        plot_cuts_score(denoised, text, polygon, image.shape[1], image.shape[0])
        plt.savefig(join(output_path, '1-7 single_double.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-8 interior_angle.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-9 corner_residue.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-10 final.png'), bbox_inches='tight')
        plt.clf()
        ...
        plt.savefig(join(output_path, '1-0 raw.png'), bbox_inches='tight')
        plt.clf()
...
if __name__ == '__main__':
    ...
    generate_cuts(input_shape, output_dir)